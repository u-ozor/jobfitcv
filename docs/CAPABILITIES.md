...
- Track detection: classifies job as `sysadmin`, `soc`, `backend`, `cloud_devops`, `it_support`, or `ai_ml` via taxonomy embeddings in `data/taxonomy/*.npy`
...
- Converts plain-language descriptions into structured resume chunks via Claude (Haiku)
...
- **Keywords field hidden for non-skill types** — keywords on non-skill chunks are auto-generated by Claude at creation; the wizard edit form deliberately hides that field for experience/project/summary/education to prevent drift. Only skill chunks expose the keywords field.
...
- **Synthesize Summary** — POST selected chunk IDs to Claude Haiku; returns a proposed 3-sentence summary alongside the current one in a two-column modal. Approve → commits with the proposed text replacing the summary chunk. Proposed text lives only in `resume_data.json` for that variant; `resume_chunks.json` is untouched
...
- Generates tailored cover letters via Claude (claude-sonnet-4-6; override with `CL_MODEL` env var)
...
| POST | /variants/{id}/summary/synthesize | Claude Haiku summary from selected chunks; returns proposed + current |
...
The current workflow blends Claude Code, local API calls, Ollama, the Chrome extension, and Claude skills — each layer doing what it's best at. The longer-term direction is to wire these dependencies explicitly rather than relying on manual handoffs:

- **Claude Code as orchestrator** — batch review already follows a defined pipeline (Stage 0 → generate → assess → chunk review → cover letters → verdict logging). Future: codify this as an agent loop that runs the full pipeline autonomously given a batch of ingested jobs, surfacing only the decisions that require human judgment.
- **Skill-triggered pipelines** — `/batch-review` and `/pt-eval` are the first skills. As the pipeline matures, skills should map directly to pipeline stages (e.g. `/chunk-review {variant_id}` runs Stage 5 for a single job without running the full batch).
- **Tool integration layer** — the extension (capture), FastAPI (generate/assess/apply), Ollama (keyword signal), and Claude (judgment/writing) currently communicate through manual Claude Code API calls. A proper dependency graph would let Claude Code know which tools to call in which order, retry on failure, and pass outputs forward without re-derivation.
- **Outcome feedback loop** — once application outcomes are tracked (interview / rejected / offer), wire them back into chunk priority tuning automatically rather than as a manual post-batch step.

Not building any of this now — the manual pipeline is still being refined and premature automation would just lock in suboptimal patterns. This section is a map of where the pieces connect.
